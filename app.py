...
app.config["AWS_COGNITO_REDIRECT_URL"] = "https://aalolabs.com/postlogin"
app.config["AWS_COGNITO_LOGOUT_URL"] = "https://aalolabs.com/postlogout"
...
app.config['SECRET_KEY'] = 'aalolabssessionkey'
Session(app)
db = SQLAlchemy(app)
...
@cognito_login
def login():
    session['code_verifier'] = auth.get_code_verifier()
    return redirect(auth.get_sign_in_url())
...
def postlogin():
    print("Session state:", session.get('cognito_state'))
...
def get_json_files(app_id):
    ...
    try:
        ...
        for file_path in files:
            ...
            current[parts[-1]] = file_path  # Store the full path as the value
